  ...
  const playAtIndex = (idx: number) => {
    if (!music.length || idx < 0 || idx >= music.length) {
      toast({
        ...
        description: 'Unable to locate the selected track in the pending list.',
        variant: 'destructive',
      });
      return;
    }
                    ...
                    <tbody>
                      {music.map((musicItem) => (
                        <tr key={musicItem.id} className="hover:bg-muted/50">
                          ...
                          <td className="border border-border p-3">{getMediaTypeLabel(musicItem.fileType, musicItem.fileUrl, musicItem.artistUploadType?.typeName)}</td>
                          ...
                          <td className="border border-border p-3">
                            <div className="flex flex-wrap gap-2">
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => playAtIndex(music.findIndex((m) => m.id === musicItem.id))}
                              >
                ...
                {music.map((musicItem) => (
                  ...
                  >
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-semibold uppercase text-muted-foreground tracking-wide">Title</span>
                      <span className="text-base font-semibold text-foreground break-words">{musicItem.title}</span>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-semibold uppercase text-muted-foreground tracking-wide">Artist</span>
                      <span className="text-sm text-foreground break-words">{musicItem.artist || musicItem.user?.email}</span>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-semibold uppercase text-muted-foreground tracking-wide">Album</span>
                      <span className="text-sm text-foreground break-words">{musicItem.albumName || '-'}</span>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-semibold uppercase text-muted-foreground tracking-wide">Type</span>
                      <span className="text-sm text-foreground">{getMediaTypeLabel(musicItem.fileType, musicItem.fileUrl, musicItem.artistUploadType?.typeName)}</span>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-semibold uppercase text-muted-foreground tracking-wide">Duration</span>
                      <span className="text-sm text-foreground">{formatDuration(musicItem.duration)}</span>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-semibold uppercase text-muted-foreground tracking-wide">Upload Date</span>
                      <span className="text-sm text-foreground">{new Date(musicItem.uploadedDate).toLocaleDateString()}</span>
                    </div>
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-semibold uppercase text-muted-foreground tracking-wide">Status</span>
                      <div>{getStatusBadge((musicItem.status?.statusName || (musicItem as any).status?.status || 'PENDING'))}</div>
                    </div>
                    ...
                    <div className="pt-2 border-t border-border/50 flex flex-wrap gap-2">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        className="flex-1 min-w-[45%] text-green-600 hover:text-green-700"
                        onClick={() => {
                          setSelectedMusic(musicItem);
                          setApproveDialogOpen(true);
                        }}
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        className="flex-1 min-w-[45%] text-red-600 hover:text-red-700"
                        onClick={() => {
                          setSelectedMusic(musicItem);
                          setRejectDialogOpen(true);
                        }}
