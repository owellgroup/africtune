...
const CompanyAudioPlayer: React.FC = () => {
  ...
  const filteredPlaylist = useMemo(() => {
    ...
    return playlist.filter((track) => {
      ...
      const genre = (track.artistWorkType?.workTypeName || '').toLowerCase();
      return [title, artist, album, genre].some((field) => field.includes(term));
    });
  }, [playlist, searchTerm]);
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
            ...
            <Button onClick={() => navigate('/company/logsheet/create')} className="gap-2 whitespace-nowrap">
              Create Log Sheet
            </Button>
          </div>
        </div>
          ...
          <div>
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {filteredPlaylist.length} {filteredPlaylist.length === 1 ? 'track' : 'tracks'} {searchTerm ? 'match your search' : 'available'}
                </CardDescription>
              </CardHeader>
